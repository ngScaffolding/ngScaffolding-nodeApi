...
export class MenuItemRouter {
    ...
    public async getAll(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            forkJoin([dataAccess.getMenuItems(), dataAccess.getAllWidgets() ]).subscribe(resultsCol =>{
                    ...
                    let allMenuItems = resultsCol[0];
                        ...
                        // First Root elements
                        userMenuItems.filter(menu => !menu.parent).forEach(menu => {
                        ...
                        treeMenuItems.forEach(menuItem => {MenuItemRouter.nullItemsCollection(menuItem)});
